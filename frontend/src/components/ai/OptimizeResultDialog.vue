...
import { diffResume, normalizeAiAdviceList } from "@/utils/aiDiff"
      ...
      <template v-else-if="result">
        <div class="flex items-start justify-between gap-3">
          <div>
            ...
            <h3 class="mt-1 font-semibold text-zinc-900">{{ changeCount ? '已生成 JD 优化方案' : '暂无可采纳的 JD 优化变化' }}</h3>
          ...
          <span v-if="changeCount" class="shrink-0 rounded-full border border-blue-100 bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700">{{ changeCount }} 项变化</span>
        </div>

        <div v-if="changeCount && adviceList.length" class="mt-4 grid gap-2">
          <div v-for="(item, index) in adviceList.slice(0, 3)" :key="item" class="flex gap-3 rounded-xl bg-zinc-50 px-3.5 py-3"><span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white text-[11px] font-semibold text-blue-600 shadow-sm">{{ index + 1 }}</span><p class="text-sm leading-6 text-zinc-600">{{ item }}</p></div>
        </div>

        <div v-if="changeCount" class="mt-4 flex flex-wrap gap-2">
          <Button class="h-11 w-full rounded-xl bg-zinc-950 text-white shadow-lg transition hover:-translate-y-0.5 hover:bg-blue-700" @click="reviewOpen = true">查看优化建议</Button>
        </div>
      </template>
      <div v-else class="text-sm text-zinc-500">优化结果会显示在这里。</div>
      </div>
    </div>
