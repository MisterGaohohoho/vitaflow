...
import { ArrowRight, Target, CheckCircle2, ScanSearch, Sparkles } from "lucide-vue-next"
...
const emit = defineEmits<{ "update:modelValue": [value: string]; optimize: [jd: string]; apply: []; clear: [] }>()
...
<template>
  <div class="flex h-full min-h-0 flex-col px-6 py-6 relative">
    <section v-show="!loading && !result" class="flex flex-col flex-1 min-h-0 transition-all duration-300 relative gap-2">
      ...
      <div class="relative shrink-0 py-2">
        <div class="relative flex items-center gap-3.5">
          <span class="flex h-14 w-14 shrink-0 items-center justify-center rounded-[18px] bg-white text-zinc-700 shadow-[0_8px_24px_rgba(0,0,0,0.06)] ring-1 ring-zinc-200/60" style="view-transition-name: ai-hero-icon;">
            <Target class="relative z-10 h-7 w-7" stroke-width="1.5" />
          </span>
          <div>
            <div class="text-[16px] font-semibold tracking-tight text-zinc-900">粘贴目标岗位 JD</div>
            <div class="mt-1 text-[13px] text-zinc-500">粘贴目标岗位 JD，AI 将自动提取核心要求并生成优化建议</div>
          </div>
        </div>
        <div class="relative mt-5 flex items-center gap-2.5 text-[12px] font-medium text-zinc-400">
      ...
      <div class="flex min-h-0 flex-1 flex-col relative z-10">
        <div class="flex-1 py-2 flex flex-col min-h-0">
          <label class="text-[11px] font-semibold uppercase tracking-[0.2em] text-zinc-400 shrink-0">Job description</label>
          <div class="mt-4 flex-1 flex flex-col min-h-[100px] relative transition-all duration-500 rounded-2xl bg-white/80 backdrop-blur-xl shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-white focus-within:shadow-[0_20px_60px_rgba(16,185,129,0.12)] focus-within:-translate-y-1 group">
            <textarea :value="modelValue" placeholder="粘贴完整岗位 JD，AI 将自动提取核心要求，并与你的简历进行匹配分析。" class="peer flex-1 w-full resize-none rounded-2xl border-0 bg-transparent p-5 text-[15px] leading-relaxed text-zinc-800 placeholder:text-zinc-300/80 focus:ring-0 outline-none thin-scrollbar min-h-0" @input="emit('update:modelValue', ($event.target as HTMLTextAreaElement).value)"></textarea>
          ...
          <div v-if="extractedKeywords.length" class="mt-6 shrink-0">
        ...
        <div class="shrink-0 py-4">
          <Button class="h-14 w-full rounded-full bg-zinc-900 text-[16px] font-medium text-white shadow-md transition-all active:scale-[0.98] hover:bg-zinc-800 hover:shadow-lg disabled:opacity-50 disabled:active:scale-100" :disabled="!modelValue.trim() || loading" @click="emit('optimize', modelValue)">
            <span class="mr-2 flex h-8 w-8 items-center justify-center rounded-full bg-white/15 backdrop-blur-md">
              <ScanSearch class="h-4 w-4" stroke-width="2" />
            </span>
            {{ loading ? '正在建立匹配方案...' : result ? '重新生成匹配方案' : '开始 JD 匹配' }}
          </Button>
          <p class="mt-3 text-center text-[12px] text-zinc-400">分析通常需要几十秒，完成后可逐项审阅修改</p>
        </div>
      </div>
    </section>

    <section v-if="loading || result" class="flex flex-col shrink-0 py-2 transition-all duration-300 w-full overflow-y-auto thin-scrollbar relative z-10 h-full">
      ...
      <template v-else-if="result">
        <div class="mb-5 flex flex-col gap-4">
          <div class="relative overflow-hidden rounded-[20px] bg-white p-5 shadow-[0_8px_30px_rgba(0,0,0,0.04)] ring-1 ring-zinc-200/60 flex items-center justify-between">
            <div class="flex items-center gap-4">
              <span class="flex h-12 w-12 shrink-0 items-center justify-center rounded-[16px] bg-white text-zinc-700 shadow-[0_8px_24px_rgba(0,0,0,0.06)] ring-1 ring-zinc-200/60" style="view-transition-name: ai-hero-icon;">
                <Target class="h-6 w-6" stroke-width="1.5" />
              </span>
              <div>
                <div class="text-[11px] font-semibold uppercase tracking-[0.2em] text-blue-600/80">Match Score</div>
                <div class="mt-1 flex items-baseline">
                  <span class="text-[42px] font-semibold leading-none tracking-tight text-zinc-900">{{ matchScore }}</span>
                  <span class="ml-1 text-[16px] font-medium text-zinc-400">%</span>
                </div>
              </div>
            </div>
            <div class="flex items-center gap-5 shrink-0">
              ...
              <button class="flex items-center gap-1.5 rounded-full bg-white px-3.5 py-1.5 text-[13px] font-medium text-zinc-600 ring-1 ring-zinc-200/80 shadow-[0_2px_8px_rgba(0,0,0,0.04)] transition hover:bg-zinc-50 hover:text-zinc-900" @click="emit('clear')">
                修改 JD
              </button>
            </div>
          </div>
        ...
        <div class="flex-1 min-h-0">
          <OptimizeResultDialog :result="result" :loading="false" :error="error" :current-data="currentData" @apply="emit('apply')" />
        </div>
      </template>
    </section>
  </div>
